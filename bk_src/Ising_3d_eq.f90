...
PROGRAM main
  ...
  CALL inputParams_3d_eq(l_x, l_y, l_z, beta, l_t, id_IC, id_BC, n_s)
  n_st = l_x * l_y * l_z
  CALL countSamples(n_s0)
  CALL metropolis_3d(beta, p_3d)

  ALLOCATE(r_x(1:n_st), r_y(1:n_st), r_z(1:n_st), r_p(1:n_st))
  ALLOCATE(eb(0:n_st), mb(0:n_st), IS3(0:l_x + 1, 0:l_y + 1, 0:l_z + 1), IS3_ini(0:l_x + 1, 0:l_y + 1, 0:l_z + 1))
  eb(0:n_st) = 0
  mb(0:n_st) = 0
  CALL initSp_3d(IS3_ini(0:l_x + 1, 0:l_y + 1, 0:l_z + 1))
  ...
  DO s = n_s0 + 1, n_s, 1
    ...
    !  OPEN(sl_p, file="p_sweep/pump_s"//ss//"_sweep.bin", access="stream", status="replace")
     ...
     CALL calcEn_3d(IS3_ini(0:l_x + 1, 0:l_y + 1, 0:l_z + 1), eb(0))
     mb(0) = SUM(IS3_ini(1:l_x, 1:l_y, 1:l_z))
     IS3(0:l_x + 1, 0:l_y + 1, 0:l_z + 1) = IS3_ini(0:l_x + 1, 0:l_y + 1, 0:l_z + 1)
     ...
     DO t = 1, l_t, 1
      ...
      ! WRITE(sl_ee) ee
      ! WRITE(sl_me) SUM(IS3(1:l_x, l_z / 2:l_z / 2 + 1))
