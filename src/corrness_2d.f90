PROGRAM main
  ...
  INTEGER(4), ALLOCATABLE :: eb_sq(:, :), mb_sq(:, :), ee_sq(:, :), me_sq(:, :), p(:, :)
  ...
  DO s = 1, n_s, 1
     ...
     OPEN(sl_me, file="p_sweep/pump_s"//ss//"_sweep.bin", access="stream", status="old", buffered="YES")
     
     loc = 1
     DO t = 1, l_t, 1
        READ(sl_eb, pos=loc) eb(s, t); READ(sl_mb, pos=loc) mb(s, t)
        READ(sl_ee, pos=loc) ee(s, t); READ(sl_me, pos=loc) me(s, t)
        READ(sl_@, pos=loc) p(s, t)
        loc = loc + 4
     END DO
  ...
  ALLOCATE(m_eb(1:n_s), m_ee(1:n_s), m_amb(1:n_s), m_ame(1:n_s), m_p(1:n_s))
  ALLOCATE(m_eb_sq(1:n_s), m_ee_sq(1:n_s), m_mb_sq(1:n_s), m_me_sq(1:n_s), m_p_sq(1:n_s))
  ...
  DO s = 1, n_s, 1
     ...
     IF (ac_p(s, 1) >= epsilon(0.0)) ac_p(s, 1:l_t / 2) = ac_p(s, 1:l_t / 2) / ac_ame(s, 1)
  END DO
  !$omp end parallel do
  ...
  DO s = 1, n_s, 1
     ...
     OPEN(sl_me, file="ac_p_s"//ss//".bin", status="replace", access="stream", buffered="YES")
