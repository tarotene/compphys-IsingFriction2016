...
PROGRAM main
  ...
  DO s = 1, n_s0, 1
    ...
    !  OPEN(sl_ee, file="ee_sweep/en_edge_s"//ss//"_sweep.bin", access="stream", status="replace", position="append")
    !  OPEN(sl_me, file="me_sweep/m_edge_s"//ss//"_sweep.bin", access="stream", status="replace", position="append")
    !  OPEN(sl_p, file="p_sweep/pump_s"//ss//"_sweep.bin", access="stream", status="replace", position="append")
     
     err = vslloadstreamf(str_p, "str_p_s"//ss//"_sweep.bin")
     err = vslloadstreamf(str_x, "str_x_s"//ss//"_sweep.bin")
     err = vslloadstreamf(str_z, "str_z_s"//ss//"_sweep.bin")
