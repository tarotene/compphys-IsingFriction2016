PROGRAM main
  ...
  INTEGER(4) :: sl_sp, sl_en, sl_eb, sl_ee, sl_m, sl_mb, sl_me
  ...
  CALL importParams_2d(l_x, l_z, beta, vel, l_t, id_IC, id_BC, n_s); n_st = l_x * l_z / vel
  ...
  ALLOCATE(pmp(1:vel)); eb(0:n_st) = 0; mb(0:n_st) = 0
  ...
  !$omp private(sl_sp, sl_en, sl_eb, sl_ee, sl_m, sl_mb, sl_me) &
  !$omp private(i_st, t, i_v, err, str_x, str_z, str_p, r_x, r_z, r_p, ss, st, eb, ee, mb, sp, pmp)
  DO s = n_s0 + 1, n_s, 1
     sl_en = 20 + s + 0 * n_s; sl_m = 20 + s + 1 * n_s
     sl_eb = 20 + s + 2 * n_s; sl_ee = 20 + s + 3 * n_s
     sl_mb = 20 + s + 4 * n_s; sl_me = 20 + s + 5 * n_s; sl_sp = 20 + s + 6 * n_s
     ...
     DO t = 1, l_t, 1
        DO i_v = 1, vel, 1
           ...
           WRITE(sl_en) eb(0:n_st); eb(0) = eb(n_st); WRITE(sl_m) mb(0:n_st); mb(0) = mb(n_st)
        END DO

        WRITE(st, '(i0.8)') t; OPEN(sl_sp, file="sp_sweep/sp_s"//ss//"t"//st//"_sweep.bin", access="stream", status="new")
        WRITE(sl_sp) INT1(sp(1:l_x, 1:l_z)); CLOSE(sl_sp)

        WRITE(sl_eb) eb(0); WRITE(sl_mb) mb(0)
        CALL calcEE_2d(sp(0:l_x + 1, 0:l_z + 1), ee); WRITE(sl_ee) ee
        WRITE(sl_me) SUM(sp(1:l_x, l_z / 2:l_z / 2 + 1))
        WRITE(*, *) t, SUM(pmp(1:vel)), eb(0), mb(0), ee, SUM(sp(1:l_x, l_z / 2:l_z / 2 + 1))
     END DO

     err = vsldeletestream(str_x); err = vsldeletestream(str_z); err = vsldeletestream(str_p)
     CLOSE(sl_en); CLOSE(sl_eb); CLOSE(sl_ee); CLOSE(sl_m); CLOSE(sl_mb); CLOSE(sl_me)
  END DO
  !$omp end parallel do
END PROGRAM main
