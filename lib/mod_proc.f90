  ...
  CONTAINS
    ...
    SUBROUTINE MSC_mSSFs_2d_THERMALIZE(r_l,r_x,r_z,n_st,ex,ez,wx,wz,sx,sz,nx,nz,r_a,IW2)
    ...
    SUBROUTINE MSC_mSSFs_2d_OBSERVE(r_l,r_x,r_z,n_st,ex,ez,wx,wz,sx,sz,nx,nz,r_a,IW2,d_eb,d_mb,eb,mb)
      INTEGER(4), INTENT(in) :: r_l(1:)
      INTEGER(4), INTENT(in) :: r_x(1:), r_z(1:)
      INTEGER(4), INTENT(in) :: n_st
      INTEGER(4), INTENT(in) :: ex(1:,1:), ez(1:,1:)
      ...
      INTEGER(8), INTENT(in) :: r_a(0:,0:)
      INTEGER(8), TARGET, INTENT(inout) :: IW2(1:,0:)
      INTEGER(4), INTENT(in) :: d_eb(0:,0:,0:,0:), d_mb(0:,0:)
      INTEGER(4), INTENT(inout) :: eb(0:), mb(0:)
      ...
      DO i_st = 1, n_st, 1
         ...
         !TODO: lookup table
         DO i_bit = 0, 63, 1
            eb(i_bit) = eb(i_bit) + d_eb(IBITS(a,i_bit,1),IBITS(b(0),i_bit,1),IBITS(b(1),i_bit,1),IBITS(b(2),i_bit,1))
            mb(i_bit) = mb(i_bit) + d_mb(IBITS(a,i_bit,1),IBITS(c,i_bit,1))
         END DO
      END DO
