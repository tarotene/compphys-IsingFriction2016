PROGRAM main
  ...
  CALL paramsCalc_3d(l_x, l_y, l_z, beta, vel, l_t, n_s, l_th, n_b)
  ...
  !$omp shared(ex_ee, ex_me, ex_p) &
  !$omp private(sl_eb, sl_mb, ss, loc, t) &
  !$omp private(sl_ee, sl_me, sl_p)
  DO s = 1, n_s, 1
     sl_eb = 20 + s + 2 * n_s
     sl_mb = 20 + s + 3 * n_s
  ...
  !$omp shared(ee, me, p, b_ee, b_me, b_p)
  DO s = 1, n_s, 1
  ...
  !$omp shared(ee_sq, me_sq, me_fp, b_ee_sq, b_me_sq, b_me_fp)
  DO s = 1, n_s, 1
  ...
  ! WRITE(10, '(a)') "#  l_z,     beta,  vel,       eb,    err_eb,       mb,   err_mb,       ee,   err_ee,       me,   err_me,     pump, err_pump"
  WRITE(10, '(a, i4, a, f8.4, a, i4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4)') &
  "  ", l_z, ", ", beta, ", ", vel, ", ", &
  SUM(avg_eb(1:n_s)) / DBLE(n_s), ", ", SUM(err_eb(1:n_s)) / DBLE(n_s), ", ", &
  SUM(avg_mb(1:n_s)) / DBLE(n_s), ", ", SUM(err_mb(1:n_s)) / DBLE(n_s), ", ", &
  SUM(avg_ee(1:n_s)) / DBLE(n_s), ", ", SUM(err_ee(1:n_s)) / DBLE(n_s), ", ", &
  SUM(avg_me(1:n_s)) / DBLE(n_s), ", ", SUM(err_me(1:n_s)) / DBLE(n_s), ", ", &
  SUM(avg_p(1:n_s)) / DBLE(n_s), ", ", SUM(err_p(1:n_s)) / DBLE(n_s)
  CLOSE(10)
  ...
  ! WRITE(20, '(a)') "#  l_z,     beta,  vel,       cb,    err_cb,     chib, err_chib,       ub,   err_ub,      ce,    err_ce,     chie, err_chie,       ue,   err_ue"
  WRITE(20, '(a, i4, a, f8.4, a, i4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4, a, f8.4)') &
  "  ", l_z, ", ", beta, ", ", vel, ", ", &
